...
const HistoryItem = ({history, deleteHandler}: {history: HistoryItemType, deleteHandler: (id: string) => void}) => {
    const { setQuery } = usePaginationStore();
  ...
  return (
    <div className='relative group w-full flex justify-between gap-1 items-center overflow-visible'>
        ...
        />
        <Button variant="ghost" onClick={() => setQuery(history.q)} className=' flex-1 group transition-all  group-hover:text-lg group-hover:bg-transparent  flex justify-start pl-2' size="sm">
